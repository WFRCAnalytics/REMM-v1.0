...
# table that contains pipeline buildings
@sim.table('pipeline_buildings', cache=True)
def pipeline_buildings():
	if os.path.exists("data/pipeline_buildings.csv"):
		pp = pd.read_csv("data/pipeline_buildings.csv")
		pp = pp[(pp['parcel_id'] >= 0) & (pp['DEVTYPE'].isin(['develop','demolition', 'redevelop']) == True) & (pp['year_built'] >= 2015) & (pp['year_built'] < 2100)].copy()
		pp = pp.fillna(0)
	else:
		pp = None
	return pp
